package dji.sampleV5.aircraft.poc
...
import dji.sampleV5.aircraft.poc.StreamingConfig
...
class AWSPhoneActivity : AppCompatActivity(), ConnectChecker {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.camera_phone_activity)
    ...
    private fun startNetworkMonitor() {
        ...
        monitorJob = CoroutineScope(Dispatchers.IO).launch {
            ...
            while (isActive) {
                ...
                val streamingState = try { if (this@AWSPhoneActivity::rtmpCamera2.isInitialized) rtmpCamera2.isStreaming else false } catch (_: Exception) { false }
    ...
    private fun startReconnectionRetries() {
        ...
        reconnectionJob = CoroutineScope(Dispatchers.Main).launch {
            ...
            if (tryIsStreaming()) {
                ...
                Toast.makeText(this@AWSPhoneActivity, "Reconnected", Toast.LENGTH_SHORT).show()
                streamingSince = System.currentTimeMillis()
                startStreamingTimer()
            } else {
                Log.w("MainActivity", "Could not reconnect after $reconnectionAttempts attempts")
                uiHandler.post { Toast.makeText(this@AWSPhoneActivity, "Could not reconnect. The app will keep trying in background.", Toast.LENGTH_LONG).show() }
            }
        }
    }
